...
struct asc_event_t
{
    ...
    event_callback_t callback_error; /* TODO: Will work properly only if one of read/write is assigned. NOTE!!! Callback_error should destroy event! */
    void *arg;
};

static void __asc_event_core_init_common(void);
static void __asc_event_core_destroy_common(void);
static bool __asc_event_process(asc_event_t * event, bool is_rd, bool is_wr, bool is_er);/* Returns true if event vec changes */
...
void asc_event_core_init(void)
{
    ...
    asc_assert(event_observer.fd != -1, MSG("failed to init event observer [%s]"), strerror(errno));
}

void asc_event_core_destroy(void)
{
    if (!event_observer.fd) return;
...
void asc_event_core_destroy(void)
{
    if (!event_observer.ed_vec) return;
...
void asc_event_core_loop(void)
{
    ...
    for(i = 0; i < count && ret > 0; ++i)
    {
        ...
        if (__asc_event_process(event, revents & POLLIN, revents & POLLOUT, revents & (POLLERR | POLLHUP | POLLNVAL)))
            break;/* Vector change */
    }
}
...
static void asc_event_subscribe(asc_event_t * event, bool is_add, bool is_delete)
{
    ...
    f->events = 0;
    if (event->callback_read) f->events |= POLLIN;
    if (event->callback_write) f->events |= POLLOUT;
}
...
void asc_event_core_loop(void)
{
    ...
    else if(ret > 0)
    {
        ...
        {
            asc_event_t *event = asc_ptrvector_get_at(event_observer.event_vec, i);
            if (__asc_event_process(event, FD_ISSET(event->fd, &rset), FD_ISSET(event->fd, &wset), FD_ISSET(event->fd, &eset)))
                break;/* Vector change */
        }
    }
}
...
static void asc_event_subscribe(asc_event_t * event, bool is_add, bool is_delete)
{
    ...
    if (is_delete)
    {
        ...
        {
            asc_event_t *event = asc_ptrvector_get_at(event_observer.event_vec, i);
            if (event->fd > event_observer.max_fd) event_observer.max_fd = event->fd;
        }
    } 
    else 
        if (is_add)
...
static void __asc_event_core_destroy_common(void)
{
    while (1) 
    {
        ...
        asc_assert(asc_ptrvector_count(event_observer.event_vec) < count, MSG("Events should go away during destroy, but they do not... [event %p]"), (void*) event);            
    }
...
static bool __asc_event_process(asc_event_t * event, bool is_rd, bool is_wr, bool is_er)/* Returns true if event vec changes */
{
    if (event->callback_read && is_rd)
    {
        ...
        if (event_observer.event_vec_changed) return true;
    }

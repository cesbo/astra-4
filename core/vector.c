...
static void asc_vector_grow(asc_vector_t * vec, int add_size)
{
    ...
    if (newSize <= vec->capacity_bytes) return;/* no need to grow, because have enough capacity */
...
static void asc_vector_shrink(asc_vector_t * vec, int del_size)
{
    ...
    asc_assert(newSize >= 0, "[core/vector] cannot shrink vector by %d bytes, because it has only %d bytes", del_size, vec->size_bytes);
...
asc_vector_t * asc_vector_init(int element_size)
{
    ...
    asc_assert(vec->data, "[core/vector] memory allocation failed, cannot allocate %d bytes", vec->capacity_bytes);
    return vec;
}
...
void * asc_vector_get_dataptr_at(asc_vector_t * vec, int pos_elem)
{
    ...
    asc_assert(pos_elem * vec->element_size < vec->size_bytes, "[core/vector] cannot get with pos after end");
...
void asc_vector_insert_middle(asc_vector_t * vec, int pos_elem, void * data, int count_elem)
{
    ...
    asc_assert((pos >= 0) && (pos <= vec->size_bytes), "[core/vector] cannot insert_middle at invalid pos_elem %d", pos_elem);
    if (len == 0) return;
    asc_vector_grow(vec, len);
    if (pos < vec->size_bytes)/* need to move contents */
        memmove(vec->data + vec->skip_bytes + pos + len, vec->data + vec->skip_bytes + pos, vec->size_bytes - pos);
...
void asc_vector_remove_middle(asc_vector_t * vec, int pos_elem, int count_elem)
{
    ...
    asc_assert((pos >= 0) && (pos <= vec->size_bytes), "[core/vector] cannot remove_middle with negative len");
    asc_assert((pos + len) <= vec->size_bytes, "[core/vector] cannot remove_middle with pos + len > size : %d + %d > %d", pos, len, vec->size_bytes);
    if (len == 0) return;
        
    if (pos == 0) 
        vec->skip_bytes += len;
    else if ((pos + len) < vec->size_bytes)/* need to move... */
        memmove(vec->data + vec->skip_bytes + pos, vec->data + vec->skip_bytes + pos + len, vec->size_bytes - pos - len);
...
void asc_vector_test(void)
{
    ...
    for (int i = 0; i < m; ++i)
    {
        asc_assert( ((char*)asc_vector_get_dataptr(vec)) [0] == (char)i, "Invalid data in vector, size %d", vec->size_bytes);

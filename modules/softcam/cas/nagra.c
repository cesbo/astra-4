...
static int nagra_check_caid(uint16_t caid)
{
    return ((caid & 0xFF00) == 0x1800);
}

/* check Entitlement Message */
static const uint8_t * nagra_check_em(cas_data_t *cas
                                      , const uint8_t *payload)
{
    ...
    switch(em_type)
    {
        ...
        case 0x83:
        {
            if(payload[5] == CAS2CAM(cas).ua[4]
               && payload[4] == CAS2CAM(cas).ua[5]
               && payload[3] == CAS2CAM(cas).ua[6])
            {
                if(payload[7] == 0x10) // shared
                    return payload;
                else if(payload[6] == CAS2CAM(cas).ua[7]) // unique
                    return payload;
            }
            break;
        }
...
static int nagra_check_keys(cas_data_t *cas, const uint8_t *keys)
{
...
static uint16_t nagra_check_desc(cas_data_t *cas, const uint8_t *desc)
{
